...
function remainingText(expiresAt: Date) {
  ...
  const text =
    abs >= 24 * 60 ? `${Math.ceil(abs / 60 / 24)}天` : abs >= 60 ? `${Math.ceil(abs / 60)}小时` : `${abs}分钟`;
...
app.post(
  '/api/opened-materials/:id/reprint',
  wrap(async (req, res) => {
    ...
    if (getComputedStatus(opened) === 'expired')
      return res.status(400).json({ message: '已过期物料不能补打标签，仅可废弃' });
  ...
  wrap(async (req, res) => {
    ...
    const openedMaterials = await prisma.openedMaterial.findMany({
      where: {
        ...
        ...(keyword
          ? {
              OR: [{ material: { code: { contains: keyword } } }, { material: { name: { contains: keyword } } }]
            }
          : {})
      },
      include: { material: true, organization: true },
    ...
    res.json(
      enrichedOpenedMaterials.map((item) => ({
        ...item,
...
app.get(
  ...
  wrap(async (req, res) => {
    ...
    res.json({
      ...enrichedOpened,
      remainingText: remainingText(enrichedOpened.expiresAt),
      computedStatus: getComputedStatus(enrichedOpened)
    });
  })
);
...
app.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!opened) return res.status(404).json({ message: '开封物料不存在' });
    if (getComputedStatus(opened) === 'expired')
      return res.status(400).json({ message: '已过期物料不能使用，仅可废弃' });
...
app.post(
  '/api/opened-materials/batch-scrap',
  wrap(async (req, res) => {
    const items = z
      .object({
        items: z
...
app.get(
  ...
  wrap(async (req, res) => {
    await refreshOpenedStatuses();
    const status = req.query.status ? String(req.query.status) : undefined;
    const dashboardDateRange = dateRangeWhere(
      req.query.startAt ? String(req.query.startAt) : undefined,
      req.query.endAt ? String(req.query.endAt) : undefined
    );
    const printDateRange = dateRangeWhere(
      req.query.printStartAt ? String(req.query.printStartAt) : undefined,
      req.query.printEndAt ? String(req.query.printEndAt) : undefined
    );
    const scrapDateRange = dateRangeWhere(
      req.query.scrapStartAt ? String(req.query.scrapStartAt) : undefined,
      req.query.scrapEndAt ? String(req.query.scrapEndAt) : undefined
    );
    ...
    const [
      ...
      prisma.operationLog.findMany({
        where: { ...(dashboardDateRange ? { createdAt: dashboardDateRange } : {}) },
        include: { material: true },
      ...
      prisma.printLog.findMany({
        where: { ...(printDateRange ? { createdAt: printDateRange } : {}) },
        include: { material: true, organization: true },
        orderBy: { createdAt: 'desc' },
        take: 50
      }),
      prisma.operationLog.findMany({
        where: { operationType: 'scrap', ...(scrapDateRange ? { createdAt: scrapDateRange } : {}) },
    ...
    for (const log of usageLogs) {
      if (!usageMap.has(log.materialId))
        usageMap.set(log.materialId, { material: log.material, useCount: 0, scrapCount: 0 });
    ...
    res.json({
      ...
      openedMaterials: openedMaterials.map((item) => ({
        ...item,
        remainingText: remainingText(item.expiresAt),
        computedStatus: getComputedStatus(item)
      })),
